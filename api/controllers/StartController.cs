...
namespace LiteralLifeChurch.LiveStreamingApi.controllers
{
    public class StartController : IController
    {
        ...
        public async Task<StatusOutputModel> StartServicesAsync(InputRequestModel input)
        {
            ...
            if (preRunServiceStatus.StreamingEndpoint.Status == StatusOutputModel.Status.Stopped)
            {
            ...
            foreach (string liveEventName in input.LiveEvents)
            {
                ...
                if (liveEvent.Status != StatusOutputModel.Status.Stopped)
                {
                    continue;
                }
